...
export function TopNavbar() {
  ...
  const toggleHousehold = (checked) => {
    setIsHousehold(checked);
    toast({
      title: checked ? "Household View Enabled" : "Individual View Enabled",
      description: checked
        ? "Dashboard data is now scaled for your household."
        : "Dashboard data is now isolated to your accounts.",
    });
  };
    ...
    <header className="h-14 border-b border-border bg-card flex items-center justify-between px-6 shrink-0">
      <div className="flex items-center gap-6">
        ...
        <div className="hidden md:flex items-center space-x-2 bg-muted/50 p-1.5 rounded-full border border-border/50">
          <User
            className={`w-4 h-4 ${!isHousehold ? "text-primary" : "text-muted-foreground"}`}
          />
          <Switch
            checked={isHousehold}
