...
const getMockAIResponse = (query) => {
  ...
  if (
    qStr.includes("dining") ||
    qStr.includes("food") ||
    qStr.includes("restaurant")
  ) {
    const foodTotal = mockTransactions
      .filter(
        (t) => t.category === "Food & Dining" || t.category === "Groceries",
      )
...
export function AICoPilot() {
  ...
  const [messages, setMessages] = useState([
    {
      id: "1",
      sender: "ai",
      text: "Hi there! I'm your AI Financial Co-Pilot. Ask me anything about your finances!",
    },
  ]);
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      ...
      setMessages((prev) => [
        ...prev,
        { id: (Date.now() + 1).toString(), sender: "ai", text: response },
      ]);
      setIsTyping(false);
    }, 1500);
  };
        ...
        <div className="bg-card w-[350px] sm:w-[400px] h-[500px] rounded-2xl shadow-2xl border border-border flex flex-col mb-4 overflow-hidden animate-in slide-in-from-bottom-4 zoom-in-95">
          ...
          <div className="bg-gradient-to-r from-primary to-primary/80 p-4 text-primary-foreground flex items-center justify-between">
            ...
            <Button
              variant="ghost"
              size="icon"
              className="h-8 w-8 text-primary-foreground hover:bg-primary/20 rounded-full"
          ...
          <div className="p-3 border-t border-border flex gap-2 items-center bg-muted/30">
            ...
            <Button
              ...
              className="bg-primary hover:bg-primary/90 text-primary-foreground shrink-0 rounded-full w-10 h-10"
            >
