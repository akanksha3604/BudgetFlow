      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-gradient-to-br from-primary/20 to-primary/5 rounded-xl border border-primary/20 p-5 flex flex-col justify-between shadow-sm">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-foreground opacity-90">
              Safe-to-Spend Today
            </span>
        ...
        <div className="bg-card rounded-xl card-shadow p-5 flex flex-col justify-between">
          ...
          <div>
            <div className="text-3xl font-bold flex items-baseline gap-2">
              ...
              <span className={`text-sm ${healthScore.color} font-medium`}>
                {healthScore.label}
              </span>
            </div>
            <div className="text-xs text-muted-foreground mt-1">
              out of 850 potential
            </div>
          </div>
        </div>
        ...
        <div className="bg-card rounded-xl card-shadow p-5 flex flex-col justify-between">
          ...
          <div className="space-y-3 mt-1">
            <div className="flex justify-between items-center text-sm border-b border-border pb-2">
              ...
              <span className="font-bold flex items-center gap-1 text-card-foreground">
                <Flame className="w-3 h-3 text-orange-500 fill-orange-500" />{" "}
                {streaks.budgetStreak} mo
              </span>
            </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Estimated monthly CO₂ emissions based on your spending
                categories
              </p>
            </div>
          </div>
          <div className="flex flex-col items-end w-full sm:w-auto">
            ...
            <div className="w-full sm:w-48 bg-muted rounded-full h-1.5 mt-1 relative overflow-hidden">
              <div
                className="bg-green-500 h-1.5 rounded-full absolute left-0 top-0 transition-all"
                style={{ width: "45%" }}
              />
            </div>
          </div>
        ...
        <div className="space-y-3">
          {mockTransactions.slice(0, 6).map((t) => (
            ...
            >
              <div className="flex items-center gap-3">
                <div
                  className={`w-9 h-9 rounded-lg flex items-center justify-center text-sm ${t.type === "income" ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"}`}
                >
                  {t.type === "income" ? (
      ...
      <div className="bg-card rounded-xl card-shadow p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {mockAccounts.map((acc) => (
            <div
              key={acc.id}
              className={`p-4 rounded-lg border border-border ${!acc.active ? "opacity-50" : ""}`}
            >
              ...
              <div
                className={`text-lg font-bold ${acc.balance >= 0 ? "text-success" : "text-destructive"}`}
              >
                {fmt(acc.balance)}
              </div>
            </div>
